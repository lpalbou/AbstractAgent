...
def test_base_dependency_floors_match_gateway_alignment() -> None:
    ...
    assert "abstractcore[tools]>=2.13.32" in deps
    assert "abstractruntime>=0.4.27" in deps
...
def test_hardware_profile_extras_are_core_runtime_cascades() -> None:
    ...
    assert set(extras["apple"]) == {
        "abstractcore[all-apple]>=2.13.32",
        "abstractruntime[apple]>=0.4.27",
    }
    assert set(extras["gpu"]) == {
        "abstractcore[all-gpu]>=2.13.32",
        "abstractruntime[gpu]>=0.4.27",
    }
