...
def ensure_react_vars(run: RunState) -> tuple[Dict[str, Any], Dict[str, Any], Dict[str, Any], Dict[str, Any]]:
...
def create_react_workflow(
    *,
    ...
    def init_node(run: RunState, ctx) -> StepPlan:
        context, scratchpad, runtime_ns, _ = ensure_react_vars(run)
        scratchpad["iteration"] = 0
    ...
    def parse_node(run: RunState, ctx) -> StepPlan:
        context, _, _, temp = ensure_react_vars(run)
    ...
    def act_node(run: RunState, ctx) -> StepPlan:
        _, _, _, temp = ensure_react_vars(run)
    ...
    def done_node(run: RunState, ctx) -> StepPlan:
        context, scratchpad, _, temp = ensure_react_vars(run)
    ...
    def max_iterations_node(run: RunState, ctx) -> StepPlan:
        context, scratchpad, _, _ = ensure_react_vars(run)
        max_iterations = int(scratchpad.get("max_iterations") or 20)
        if max_iterations < 1:
            max_iterations = 1
